...
namespace SDLGlue
{
	...
	internal static class SDL
	{
		...
		private static readonly string[] ExtractFileNames = { "README-SDL.txt", "SDL2.dll" };

		static SDL()
		{
			var dir = ExtractLibraries();
	    ...
	    private static string ExtractLibraries()
		{
	        ...
	        var path = thisAssembly.Location;

            var outputPath = Path.Combine(Directory.GetParent(path).FullName, "Native", "SDL2", Environment.Is64BitProcess ? "x64" : "x86");
            Directory.CreateDirectory(outputPath);

			var resourcePrefix = Environment.Is64BitProcess ? "SDLGlue.Libs.x64." : "SDLGlue.Libs.x86.";

			foreach (var file in ExtractFileNames)
			{
